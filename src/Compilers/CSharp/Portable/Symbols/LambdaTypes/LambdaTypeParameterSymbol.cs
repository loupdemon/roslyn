...
{
    /// <summary>A parameter to a lambda type. *Not* a type parameter to a lambda.</summary>
    internal sealed class LambdaTypeParameterSymbol : ParameterSymbol
    {
        private readonly LambdaTypeMethodSymbol _containingSymbol;

        public LambdaTypeParameterSymbol(TypeWithAnnotations typeWithAnnotations, RefKind refKind, int ordinal, string name, LambdaTypeMethodSymbol containingSymbol, ImmutableArray<CustomModifier> refCustomModifiers)
        {
        ...
        public override bool Equals(Symbol other, TypeCompareKind compareKind)
        {
            ...
            if (!(other is LambdaTypeParameterSymbol param))
            {
                return false;
            }
        ...
        internal bool Equals(LambdaTypeParameterSymbol other, TypeCompareKind compareKind)
            => other.Ordinal == Ordinal
               && _containingSymbol.Equals(other._containingSymbol, compareKind);

        internal bool MethodEqualityChecks(LambdaTypeParameterSymbol other, TypeCompareKind compareKind)
